...
export class cdkStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps, amplifyResourceProps?: AmplifyHelpers.AmplifyResourceProps) {
    ...
    const plan = new backup.BackupPlan(this,
      "ReSkillDbBackupPlan",
      {
        backupPlanName: `ReSkillBackupPlan${cdk.Fn.ref('env')}`,
        backupPlanRules: [
          backup.BackupPlanRule.daily(),
          backup.BackupPlanRule.weekly(),
          backup.BackupPlanRule.monthly1Year()
        ],
        backupVault: new backup.BackupVault(this, 'Vault', { backupVaultName: `ReSkillBackupVault${AmplifyHelpers.getProjectInfo().envName}` })
      }
    );
